...
- [~] Add `RAGMode` type and `RAG_MODE` env var to `internal/config/env.go`
- [ ] Create `application/service/rag.go` with:
...
- [ ] Expose `RAG *service.RAG` on `kodit.Client` and wire it in `kodit.go`
- [ ] Write tests for `RAG.Convert` and `RAG.Index` (mock repository service)
