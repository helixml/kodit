...
- [x] Write unit tests in `infrastructure/extraction/csv_test.go` covering: header row, string-only columns, numeric columns skipped, deduplication, top-5 rows, no-header CSV, empty file
- [x] Add `.csv: true` to `indexableExtensions` in `application/handler/indexing/chunk_files.go`
