...
- [ ] Add `PATCH /{id}` route to `RepositoriesRouter` in `infrastructure/api/v1/repositories.go` accepting `chunk_size`, `chunk_overlap`, and `chunk_min_size`; validate `overlap < size` and all values positive; return 400 on invalid input
- [ ] Update repository GET and List DTOs in `infrastructure/api/v1/dto/` to include `chunk_size`, `chunk_overlap`, and `chunk_min_size` (nullable)
