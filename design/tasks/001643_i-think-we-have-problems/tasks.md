...
- [x] Add a `gitBatchSize = 1000` constant (e.g. in `infrastructure/persistence/batch.go` or near the existing `saveAllBatchSize`)
- [x] Fix `FileStore.SaveAll`: replace `Create(&models)` with `CreateInBatches(models, gitBatchSize)` (keep `OnConflict` clause)
