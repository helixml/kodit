...
func (h *ScanCommit) Execute(ctx context.Context, payload map[string]any) error {
	...
	if existing {
		h.logger.Info("commit already scanned",
			...
			slog.String("commit", shortSHA(commitSHA)),
		)
	...
	savedCommit, err := h.commitRepo.Save(ctx, commit)
	if err != nil {
		if failErr := tracker.Fail(ctx, err.Error()); failErr != nil {
			h.logger.Warn("failed to mark tracker as failed", slog.String("error", failErr.Error()))
		}
	...
	files := result.Files()
	if len(files) > 0 {
		if _, err := h.fileRepo.SaveAll(ctx, files); err != nil {
