...
func NewRust(language slicer.Language) *Rust {
	return &Rust{
...
func (r *Rust) ModulePath(file slicer.ParsedFile) string {
	return r.BuildModulePathFromPath(file.Path(), ".rs")
}

// Classes extracts struct and enum definitions.
func (r *Rust) Classes(tree *sitter.Tree, source []byte) []slicer.ClassDefinition {
	if tree == nil {
		return nil
	}
...
func (r *Rust) Types(tree *sitter.Tree, source []byte) []slicer.TypeDefinition {
	if tree == nil {
		return nil
	}
...
func (r *Rust) extractStruct(node *sitter.Node, source []byte) slicer.ClassDefinition {
	nameNode := node.ChildByFieldName("name")
	name := ""
	if nameNode != nil {
...
func (r *Rust) extractImplMethods(structNode *sitter.Node, source []byte, structName string) []slicer.FunctionDefinition {
	tree := structNode
	for tree.Parent() != nil {
		tree = tree.Parent()
	}
	...
	var methods []slicer.FunctionDefinition
...
func (r *Rust) extractType(node *sitter.Node, source []byte) slicer.TypeDefinition {
	nameNode := node.ChildByFieldName("name")
	name := ""
	if nameNode != nil {
		name = r.NodeText(nameNode, source)
