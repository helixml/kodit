...
func NewPython(language slicer.Language) *Python {
	return &Python{
...
func (p *Python) IsPublic(node *sitter.Node, name string, _ []byte) bool {
...
func (p *Python) ModulePath(file slicer.ParsedFile) string {
	return p.BuildModulePathFromPath(file.Path(), ".py")
}

// Classes extracts class definitions from the AST.
func (p *Python) Classes(tree *sitter.Tree, source []byte) []slicer.ClassDefinition {
	if tree == nil {
		return nil
	}
...
func (p *Python) Types(_ *sitter.Tree, _ []byte) []slicer.TypeDefinition {
	return nil
}

func (p *Python) extractClass(node *sitter.Node, source []byte, modulePath string) slicer.ClassDefinition {
	nameNode := node.ChildByFieldName("name")
	name := ""
	if nameNode != nil {
...
func (p *Python) extractMethods(classNode *sitter.Node, source []byte, classQualifiedName string) []slicer.FunctionDefinition {
