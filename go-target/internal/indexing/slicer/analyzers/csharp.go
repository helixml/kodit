...
func NewCSharp(language slicer.Language) *CSharp {
	return &CSharp{
...
func (cs *CSharp) ModulePath(file slicer.ParsedFile) string {
	tree := file.Tree()
	if tree == nil {
		return ""
	}
...
func (cs *CSharp) Classes(tree *sitter.Tree, source []byte) []slicer.ClassDefinition {
	if tree == nil {
		return nil
	}
...
func (cs *CSharp) Types(_ *sitter.Tree, _ []byte) []slicer.TypeDefinition {
	return nil
}

func (cs *CSharp) extractClass(node *sitter.Node, source []byte) slicer.ClassDefinition {
	nameNode := node.ChildByFieldName("name")
	name := ""
	if nameNode != nil {
...
func (cs *CSharp) extractMethods(classNode *sitter.Node, source []byte, className string) []slicer.FunctionDefinition {
