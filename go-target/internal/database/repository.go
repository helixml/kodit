...
type TransactionalRepository[D any, E any] struct {
	Repository[D, E]
	tx *gorm.DB
}

// WithTransaction creates a repository that operates within a transaction.
func (r Repository[D, E]) WithTransaction(tx *gorm.DB) TransactionalRepository[D, E] {
	return TransactionalRepository[D, E]{
		Repository: Repository[D, E]{
			db:     r.db,
			mapper: r.mapper,
			table:  r.table,
		},
...
// Get retrieves an entity by ID within the transaction.
func (r TransactionalRepository[D, E]) Get(ctx context.Context, id int64) (D, error) {
	var entity E
	result := r.tx.Table(r.table).Where("id = ?", id).First(&entity)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			var zero D
			return zero, fmt.Errorf("%w: id %d", ErrNotFound, id)
...
// Find retrieves entities matching a query within the transaction.
func (r TransactionalRepository[D, E]) Find(ctx context.Context, query Query) ([]D, error) {
	var entities []E
	result := query.Apply(r.tx.Table(r.table)).Find(&entities)
	if result.Error != nil {
		return nil, fmt.Errorf("find entities: %w", result.Error)
	}
...
// Save creates or updates an entity within the transaction.
func (r TransactionalRepository[D, E]) Save(ctx context.Context, domain D) (D, error) {
	entity := r.mapper.ToDatabase(domain)
	result := r.tx.Table(r.table).Save(&entity)
	if result.Error != nil {
		var zero D
		return zero, fmt.Errorf("save entity: %w", result.Error)
	}
...
func (r TransactionalRepository[D, E]) Create(ctx context.Context, domain D) (D, error) {
	entity := r.mapper.ToDatabase(domain)
	result := r.tx.Table(r.table).Create(&entity)
	if result.Error != nil {
		var zero D
		return zero, fmt.Errorf("create entity: %w", result.Error)
	}
...
func (r TransactionalRepository[D, E]) Delete(ctx context.Context, domain D) error {
	entity := r.mapper.ToDatabase(domain)
	result := r.tx.Table(r.table).Delete(&entity)
	if result.Error != nil {
		return fmt.Errorf("delete entity: %w", result.Error)
	}
	return nil
