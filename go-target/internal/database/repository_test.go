...
func TestRepository_WithTransaction(t *testing.T) {
	...
	txRepo := repo.WithTransaction(txn.Session())
	...
	// Before commit, should be visible in transaction
	found, err := txRepo.Get(ctx, created.ID())
	if err != nil {
		t.Fatalf("Get in transaction: %v", err)
	}
	if found.Name() != "Alice" {
		t.Errorf("Name() = %v, want Alice", found.Name())
	}
...
func TestRepository_WithTransaction_Commit(t *testing.T) {
	ctx := context.Background()
	repo, db, cleanup := setupTestRepo(t)
	defer cleanup()
	...
	if found.Name() != "Bob" {
		t.Errorf("Name() = %v, want Bob", found.Name())
	}
}
