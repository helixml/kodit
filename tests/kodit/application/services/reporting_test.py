...
async def test_nested_progress_trackers_share_parent_info() -> None:
    """Test the reporting."""
    progress_tracker = ProgressTracker.create(OperationType.ROOT.value)
    ...
    assert progress_tracker.progress.trackable_id == 1
    assert progress_tracker.progress.trackable_type == TrackableType.INDEX
    assert child_progress_tracker.parent == progress_tracker
    assert child_progress_tracker.progress.trackable_id == 1
    assert child_progress_tracker.progress.trackable_type == TrackableType.INDEX
...
async def test_progress_tracker_state_flow() -> None:
    ...
    async with progress_tracker:
        assert progress_tracker.progress.state == ReportingState.STARTED
        await progress_tracker.set_total(100)
        await progress_tracker.set_current(50)
        assert progress_tracker.progress.state == ReportingState.IN_PROGRESS
    assert progress_tracker.progress.state == ReportingState.COMPLETED
    assert progress_tracker.progress.total == 100
