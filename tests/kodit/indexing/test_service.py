...
@pytest.mark.asyncio
async def test_run_index_not_found(service: IndexService):
    """Test running a non-existent index."""
    with pytest.raises(ValueError, match="Index not found"):
        await service.run(999)


@pytest.mark.asyncio
async def test_run_index_source_not_found(service: IndexService, session: AsyncSession):
