...
from kodit.domain.protocols import OperationRepository, UnitOfWork
...
class SqlAlchemyOperationRepository(OperationRepository):
    ...
    @property
    def _mapper(self) -> OperationMapper:
        """Get the operation mapper using the current session."""
    ...
    async def get_by_index_id(self, index_id: int) -> OperationAggregate:
        ...
        async with self.uow:
            ...
            db_operation = await self.uow.session.scalar(stmt)
            ...
            current_step = await self.uow.session.scalar(step_stmt)
    ...
    async def save(self, operation: OperationAggregate) -> None:
        ...
        async with self.uow:
            ...
            if db_operation:
                ...
                self.uow.session.add(db_operation)
                await self.uow.session.flush()  # Get operation ID
            ...
            if operation.current_step:
                ...
                db_step = await self.uow.session.scalar(step_stmt)

                if db_step:
                    # Update existing step
                    db_step.state = operation.current_step.state.value
                    db_step.progress_percentage = (
                        operation.current_step.progress_percentage
                    )
