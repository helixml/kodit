...
class VectorChordBM25Repository(BM25Repository):
    ...
    async def _create_extensions(self) -> None:
        ...
        async with self.uow:
            ...
            await self.uow.session.execute(text(CREATE_PG_TOKENIZER))
            await self.uow.session.execute(text(CREATE_VCHORD_BM25))
            await self.uow.session.execute(text(SET_SEARCH_PATH))
    ...
    async def _create_tokenizer_if_not_exists(self) -> None:
        ...
        async with self.uow:
            result = await self.uow.session.execute(text(TOKENIZER_NAME_CHECK_QUERY))
            if result.scalar_one_or_none() is None:
                # Tokenizer doesn't exist, create it
                await self.uow.session.execute(text(LOAD_TOKENIZER))
                await self.uow.commit()

    async def _create_tables(self) -> None:
        """Create the necessary tables in the correct order."""
        async with self.uow:
            await self.uow.session.execute(text(CREATE_BM25_TABLE))
            await self.uow.session.execute(text(CREATE_BM25_INDEX))
            await self.uow.commit()
    ...
    async def index_documents(self, request: IndexRequest) -> None:
        ...
        async with self.uow:
            ...
            await self.uow.session.execute(text(UPDATE_QUERY))
            await self.uow.commit()
    ...
    async def search(self, request: SearchRequest) -> list[SearchResult]:
        ...
        async with self.uow:
            result = await self.uow.session.execute(sql)
