"""Service for search indexing operations (BM25 and embeddings)."""
...
class SearchIndexingService:
    """Handles search indexing operations including BM25 and embeddings."""
    ...
    async def create_bm25_index(self, repository_id: int, commit_sha: str) -> None:
        """Create BM25 keyword index for commit snippets."""
    ...
    async def create_code_embeddings(
        self, repository_id: int, commit_sha: str
    ) -> None:
        """Create code embeddings for commit snippets."""
        async with self.operation.create_child(
    ...
    async def create_summary_embeddings(
        self, repository_id: int, commit_sha: str
    ) -> None:
        """Create text embeddings for snippet summaries."""
        async with self.operation.create_child(
    ...
    async def _new_snippets_for_type(
        ...
        """Get new snippets that don't have embeddings of the given type yet."""
        existing_embeddings = (
