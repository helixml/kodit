"""Service for scheduling periodic sync operations (V2).

This version uses the lightweight V2 repositories for improved performance.
"""
...
from kodit.domain.protocols_v2 import GitRepositoryRepositoryV2
...
class SyncSchedulerServiceV2:
    """Service for scheduling periodic sync operations using V2 repositories."""

    def __init__(
        self,
        queue_service: QueueService,
        repo_repository: GitRepositoryRepositoryV2,
    ) -> None:
    ...
    async def _perform_sync(self) -> None:
        ...
        This is much more efficient than the original version since
        get_all() only returns repository metadata, not all commits,
        branches, tags, and files.
        """
        self.log.info("Starting sync operation")

        # Sync each index - queue all 5 tasks with priority ordering
        # This now only loads lightweight repository metadata
