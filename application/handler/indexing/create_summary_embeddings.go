...
func (h *CreateSummaryEmbeddings) Execute(ctx context.Context, payload map[string]any) error {
	...
	enrichments, err := h.enrichmentStore.FindByCommitSHA(ctx, cp.CommitSHA(), enrichment.WithType(enrichment.TypeDevelopment), enrichment.WithSubtype(enrichment.SubtypeSnippetSummary))
	if err != nil {
